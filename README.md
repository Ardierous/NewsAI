...
| Точка | Дата | Что изменилось |
|-------|------|----------------|
| **А** | 19.08.2026 | Пакет **«практичный дайджест»**: поиск и отбор материалов с прикладной пользой (обзоры, сравнения, кейсы), а не только новости о релизах моделей. |
| **Б** | 20.08.2026 | **Объединение стилей:** «Серьёзный» и «Курьёзный» сливаются в один режим **«Дайджест ИИ»** — смешанный пул без выбора типа на шаге 0. |

**Как пользоваться метками в переписке**

- **«До точки А»** — до пакета «практичный дайджест» (19.08.2026): другой баланс источников, фильтров и подсказок на шаге 1.
- **«Точка А» / «как на А»** — после практичного пакета, но **ещё два типа** на шаге 0 (Серьёзный / Курьёзный).
- **«Между А и Б»** — точечные фиксы после А (Telegram, ручные URL, заголовки, UTM); тип выпуска на шаге 0 не менялся.
- **«Точка Б» / «после точки Б»** — один «Дайджест ИИ», объединённый сбор serious + curious на шаге 1.

### Точка А (19.08.2026) — что изменилось

Зафиксированный набор правок под **практичный выпуск**: в дайджесте должны чаще попадать материалы «как применить ИИ в работе, учёбе и быту», а не только сухие новости о моделях и релизах.

**Поиск и источники**

- В `source_tiers.txt` добавлены и усилены прикладные площадки (`lifehacker.ru`, `rb.ru`, `cnews.ru`) — в tier-списки и seed-URL.
- В `digest_service.py` — отдельный rescue-запрос `_step1_practical_tools_query` в цепочке добора для серьёзного выпуска (если обычная выдача «сухая» или старая).

**Фильтры и ранжирование**

- В `step1_candidate_policy.py` — сигнал `has_practical_review_signal`: качественные обзоры и сравнения не отсекаются как `product_tool_promo`, если это не лендинг и не чистая реклама.
- В `digest_contract.txt` — приоритет практической пользы при равной новостной значимости.

**Документация и интерфейс**

- Маркер **точки А** в README; описание практичного уклона шага 1.
- Подсказки в `DigestWizard.tsx`: на шагах 1–2 — про прикладные обзоры и ручные ссылки; на шаге 3 — как оценивать практичность через блоки «Польза» и «Аудитория».

**Что при этом не менялось**

- На шаге 0 по-прежнему **два типа** выпуска про ИИ: **Серьёзный** и **Курьёзный** (раздельный поиск и фильтры).
- Тема «Стиль», шаги 2–4, общая схема мастера — без изменений.

### Между А и Б (19–20.08.2026)

Доработки поверх точки А; выбор Серьёзный/Курьёзный на шаге 0 **сохранялся**:

- Telegram: прямой парсинг `t.me/s/…` вместе с ProxyAPI (ссылки из каналов, в т.ч. Технократия).
- Шаг 2: блок ручных URL под пулом; сохранение выбора при добавлении ссылки.
- Шаг 3: заголовок новости перед «Суть новости».
- Пул: восстановление обрезанных заголовков; очистка URL от UTM/query.

### Точка Б (20.08.2026) — объединение серьёзного и курьёзного стилей

**Суть:** редактор больше не выбирает «деловой» или «курьёзный» выпуск. Один режим **«Дайджест ИИ»** сам собирает **смешанный** пул: деловые новости, практичные обзоры и умеренно развлекательные истории про ИИ.

| | После точки А | После точки Б |
|---|---------------|---------------|
| Шаг 0 | Серьёзный / Курьёзный / По умолчанию | Одна кнопка «Сохранить параметры шага 0», подпись «Дайджест ИИ» |
| Поиск | Два контура: tier для serious, `curious_source_hosts` для curious | Один контур: tier для всех + **автодобор** курьёзных и практичных источников |
| Фильтры | Профиль `curious` с `off_topic_not_curious` | Для новых выпусков — профиль `serious` |
| Финал (новые выпуски) | Заголовок/лид от выбранного типа | «Пять новостей про ИИ», смешанный лид |

**Технически**

- `digest_type_policy.py`: legacy `curious` нормализуется в `serious`; отдельный курьёзный контур шага 1 отключён.
- Шаг 1: `serious_tier` + добор (`serious_use_curious_tiers`, rescue: practical → curious → fresh tier-1).
- Старые выпуски с `digest_type=curious` в БД: финал шага 4 может остаться курьёзным; новый сбор шага 1 — уже по объединённым правилам.
- Плашки «Серьёз / Курьёз» на карточках — угол **материала**, не режим выпуска.

**Обновлено вместе с объединением:** UI мастера и памятки, `step1_search_routing.py`, rescue в `digest_service.py`, заголовки в `platform_assembly.py`, README, `STEP1_PIPELINE.md`, `AGENTS.md`, целевые тесты.
...
backend/
  app/
    ...
    step1_filter_settings.json   # порог воронки, порядок фильтров (единый «Дайджест ИИ» = serious)
...
|-----|---------------------|--------------|--------|
| 0 | `step_0` | Окно дат + режим «Дайджест ИИ» | Сохраняет параметры; канонический тип — **serious** (`digest_defaults.json`); legacy `curious` нормализуется |
...
- Поиск ориентирован на практичность: в приоритете материалы с прикладной пользой (обзоры, сравнения и сценарии применения ИИ в работе/учёбе/быту), а не только релизы моделей.
