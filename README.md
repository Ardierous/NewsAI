...
backend/
  app/
    ...
    services/
      ...
      export_service.py, reader_copy.py, platform_assembly.py, …
...
- «Оптимально по мнению ИИ» расставляет пятёрку и пишет аргументацию по позициям; шаг 3 не запускает — после правок нажмите «Применить порядок».
...
- `backend/storage` (SQLite база `storage/digest.db`, экспорты, служебные JSON)
- `backend/logs` (логи backend)

### Публикация в Docker Hub

- Windows: `scripts\push_dockerhub.bat`
