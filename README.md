...
| 1 | `step_1_candidates` | «Запустить сбор кандидатов» (опц. manual URL) | Поиск URL, HTTP-верификация, пул 10–15 |
| 2 | `selected` | Выбор 5 + «Применить порядок» / «Оптимально по мнению ИИ» | Сохраняет пятёрку и порядок |
| 3 | `analytics_ready` | Обычно автоматически после шага 2; иначе «Запустить аналитику» | AnalyticsAgent: карточки + хэштеги |
| 4 | `final_ready` | «Сгенерировать тексты» (+ обложки при включённом флаге) | Тексты площадок, QC, `.docx` |

**Важно:**

- Окно дат передаётся при каждом `POST …/step1/run` и может быть изменено через `PATCH …/news-window` без сброса шага 0.
- После «Применить порядок» шаг 3 запускается автоматически, если `AUTO_RUN_STEP3_AFTER_ORDER=true` (по умолчанию).
- Шаг 4 в UI разбит: `generate-images` → `select-image` → `generate-texts`. Эндпоинт `confirm-final` — legacy-монолит для совместимости.
- «Пересобрать пул кандидатов» → `POST …/step1/run` с `"rebuild": true`.
...
- `POST /digests/{id}/step2/select` — выбор 5 (`selected_ids` или `top5: true`)
- `POST /digests/{id}/step2/order` — порядок + опционально авто-шаг 3
- `POST /digests/{id}/step2/order/ai-optimal` — порядок через ProxyAPI (без Crew)
...
2. Шаг 0 → шаг 1 (дождаться пула ≥10) → выбор 5 → порядок → аналитика → тексты шага 4.
3. Проверить «Скачать .docx» и блок «Шаг 4 — результат».
