...
def create_agents(system_contract: str) -> CrewAgents:
    ...
    return CrewAgents(
        news_research=Agent(
            ...
            backstory="Ты редактор-исследователь, работаешь строго по контракту и учитываешь Tier источников.",
            llm=llm,
        ...
        source_verification=Agent(
            ...
            backstory="Ты фактчекер редакции ExTellect, формируешь прозрачный статус верификации.",
            llm=llm,
            allow_delegation=False,
            verbose=False,
        ),
