...
_ANALYTICS_EDITORIAL = (
    ...
    "essence — одна строка в формате «Суть новости» для быстрого просмотра в UI. "
    "analysis верни СТРОГО в 5 блоках и в этом порядке, каждый блок с новой строки и с меткой: "
    "«Аудитория: ...», «Почему важно: ...», «Польза: ...», «Вред: ...», «Возможные последствия: ...». "
    "В каждом блоке 1–2 простых предложения по фактам статьи, без воды и без служебных меток. "
...
def _fallback_analytics_item(news: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "analysis": (
            "Аудитория: Команды и специалисты, на которых напрямую влияет эта новость. "
            "Почему важно: Материал может повлиять на рабочие решения и приоритеты в теме ИИ. "
            "Польза: Можно использовать выводы новости как ориентир для своих задач и планирования. "
            "Вред: При поспешных выводах есть риск неверной интерпретации без сверки с первоисточником. "
            "Возможные последствия: Стоит проверить факты и оценить, как изменения отразятся на вашей работе."
        ),
    }
