...
def _make_service(monkeypatch: pytest.MonkeyPatch) -> tuple[DigestService, Digest]:
    ...
    monkeypatch.setattr(service.workflow, "run_candidates_research", lambda digest_type, now_msk, manual_urls: "[]")
    monkeypatch.setattr(service.workflow, "run_candidates_verify", lambda research_raw: "[]")
...
def test_step1_persists_reject_reasons_on_502(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(service.workflow, "run_candidates_score", lambda verify_raw, now_msk: score_rows)
